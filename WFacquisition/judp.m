...
if strcmpi(actionStr,'send')
    ...
    if nargin < 4
        errorcd /Users/kpb/Documents/field/2021/2021-07-13_drone_test/data/2021-07-14_quadcopter3_log_upload/converted_using_mission_planner('judp:Syntax',[mfilename '.m--SEND mode requires 4 input arguments.']);
    end % if
    ...
    if length(port) == 1
        ...
        targetPort = port;
    elseif length(port) == 2
        sourcePortSpecified = true;
        targetPort = port(1);
        sourcePort = port(2);
    else
        error('judp:Syntax',[mfilename '.m--SEND mode requires PORT to consist of either one or two integers.']);
    end 
    
elseif strcmpi(actionStr,'receive')
    % sourcePort is never specified for receive mode.
    sourcePortSpecified = false;
    action = RECEIVE;
    if nargin < 3
        error('judp:Syntax',[mfilename '.m--RECEIVE mode requires 3 input arguments.']);
    end % if
    ...
    error('judp:Syntax',[mfilename '.m--Unrecognised actionStr ''' actionStr ''.']);
end % if
% Test validity of input arguments.
if sourcePortSpecified
    if length(sourcePort)~=1 || ~isnumeric(sourcePort) || rem(sourcePort,1)~=0 || sourcePort < 1025 || sourcePort > 65535
        error('judp:BadPortNumber',[mfilename '.m--Source port number must be an integer between 1025 and 65535.']);
    end % if
end % if
if length(targetPort)~=1 || ~isnumeric(targetPort) || rem(targetPort,1)~=0 || targetPort < 1025 || targetPort > 65535
    error('judp:BadPortNumber',[mfilename '.m--Target port number must be an integer between 1025 and 65535.']);
end % if
if action == SEND
    if ~ischar(host)
        error('judp:Syntax',[mfilename '.m--Host name/IP must be a string (e.g., ''www.examplecom'' or ''208.77.188.166''.).']);
    end % if
    
    if ~isa(mssg,'int8')
        error('judp:BadMessage',[mfilename '.m--Mssg must be int8 format.']);
    end % if
    
elseif action == RECEIVE    
    
    if ~isnumeric(packetLength) || rem(packetLength,1)~=0 || packetLength < 1
        error('judp:Syntax',[mfilename '.m--packetLength must be a positive integer.']);
    end % if
    
    if ~isnumeric(timeout) || timeout <= 0
        error('judp:Syntax',[mfilename '.m--timeout must be positive.']);
    end % if    
...
if action == SEND
    try
        ...
        packet = DatagramPacket(mssg, length(mssg), addr, targetPort);
 
        if exist('sourcePort')
            socket = DatagramSocket(sourcePort);
        else
