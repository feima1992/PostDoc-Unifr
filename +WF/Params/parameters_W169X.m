...
arguments
    folderName(1,:) char = 'WF_W169X'; % folder name of the analysis
end
...
p.note = 'WF analysis: mouse 1693, mouse 1694';
...
p.dir.ACTmaps = fullfile(p.folder,'ACTraw'); % directory of activation maps
...
% experiment type
p.select.stimType.proprioception = 0; % passive movement session
p.select.stimType.vibration = 1; % vibration session
p.select.stimType.whisker = 0; % whisker session

% animal to be analysed
p.select.animal.proprioception = {};
p.select.animal.vibration = {'w1693','w1694'};
...
p.select.trial.proprioception.moveDirection = [1,2,3,4,5,6,7,8]; % direction 1,2,...,8 seperately and all directions together
