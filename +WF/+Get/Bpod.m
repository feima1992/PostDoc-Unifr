function bpodInfo = BpodInfo(p)
    fprintf('▶  Get Bpod files information\n')
    % scan the bpod folder to find all the files
    filesBpod = FindFiles(p.dir.bp, '.mat', {'DefaultSettings', 'LimbReachGrasp', 'FakeSubject'}, 'table_output', true);
    ...
    filesBpod = SelectSession(filesBpod, p);
    % process sessions with multipul recordings
    filesBpod = RemoveDuplicate(filesBpod);
    % load bpod files
    bpodInfo = table();

    for i = progress(1:height(filesBpod), 'Title', '   Loading Bpod')
...
function filesList = SelectSession(filesBpod, p)
    ...
    if p.select.stimType.proprioception
        ...
        sessionID = p.select.session.proprioception + 20000000;
