...
fileList = struct2table(F.FindFiles(folder,{'subgroup','.mat'},{'2-1','3-1'}));
...
idx = strcmp(fileList.groupPairs,'7-4');
fileList.subgroupID(idx) = fileList.subgroupID(idx) + 6;
...
% create a cell array to store the data, each column is a mouse, each row is a subgroup
...
session = {'R&G(1W)','R&G(2W)','R&G(3W)','R&G(4W)','R&G(5W)','R&G(7W)','Wash(1W)','Wash(2W)','Wash(3W)'};
...
%dataFs1Mask = cellfun(@(X)WFA.ApplyLabelMask(X,[23,25,26,29,30]),data,'UniformOutput',false);
dataFs1Mask = cellfun(@(X)WFA.ApplyLabelMask(X,26),data,'UniformOutput',false);
meanFs1Diff = cellfun(@(X)min(X(:)),dataFs1Mask,'UniformOutput',false);
meanFs1Diff = cell2mat(meanFs1Diff);
...
xticks(1:9);
% xticklabels
xticklabels({'R&G(1W)','R&G(2W)','R&G(3W)','R&G(4W)','R&G(5W)','R&G(7W)','Wash(1W)','Wash(2W)','Wash(3W)'});
% xtickangle
xtickangle(45);
% vertical line at x = 6
hLin = xline(6,'--','LineWidth',2);
